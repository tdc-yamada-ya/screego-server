このプロジェクトは WebRTC でスクリーン共有を行うアプリケーションである [screego](https://github.com/screego/server) をフォークし、カスタマイズを加えたものです。
カスタマイズ内容は以下の通りです。

- 画面共有を行う際にデスクトップ音声も共有する
- 共有されたデスクトップ音声のオンオフを行う
- 最大ビットレート、フレームレート、画面共有の幅、高さ、音声の各種フラグの設定項目を追加
